...
- **发现者**: ciyeer
- **Bug 描述**: 
    - QObject: Cannot create children for a parent that is in a different thread.(Parent is QSerialPort(0x1a090aea270), parent's thread is QThread(0x1a08c566960), current thread is QThreadPoolThread(0x1a090bb75b0)
- **严重级别**: 高
- **复现步骤**:
  1. 程序运行即可出现

- **问题分析**: 
    - 这是因为 `QSerialPort` 不能跨线程创建子对象。`QSerialPort` 被创建在一个线程（`QThread(0x1a08c566960)`）中，而它的子对象或者它的操作被放入另一个线程（`QThreadPoolThread`）中，这会导致 `QObject` 相关的父子关系出现问题。
    在 Qt 中，`QObject`（包括 `QSerialPort`）默认属于创建它的线程，也就是说 `QSerialPort` 对象必须与其所在的线程（`QThread`）一致。如果您在工作线程中尝试访问或创建 `QSerialPort`，就会触发这个错误。
- **解决方案**: 
    - 在子线程中使用 `QSerialPort`，需要将其对象移动到相应的线程中。可以使用 `moveToThread()` 方法将 `QSerialPort` 对象从主线程移到工作线程。
- **状态**: 
    - 已修复
