...
### 2.1 Hex 转换优化 (#7)
- [x] 2.1.1 将 `BytetraceBase::onSerialDataReceived()` 中的循环 `QString::arg()` 拼接替换为 `QByteArray::toHex(' ')`
- [x] 2.1.2 同样优化发送端的 HEX 转字节逻辑（如存在类似问题）

### 2.2 串口轮询优化 (#8)
- [x] 2.2.1 在 `updateUI(false)` 中调用 `m_portMonitorTimer->stop()`
- [x] 2.2.2 在 `updateUI(true)` 中调用 `m_portMonitorTimer->start()`
- [x] 2.2.3 保留 `errorOccurred` 信号处理作为双重保险

## 3. P1 — 代码质量清理

### 3.1 内联样式迁移 (#9, #10)
- [x] 3.1.1 移除 `bytetracebase.cpp` 中 `setStyleSheet("background-color: #FF6B6B")` 和 `#4CAF50`
- [x] 3.1.2 移除 `settingsui.cpp` 中的 `setStyleSheet` 调用
- [x] 3.1.3 在 `default.qss` 中添加对应规则（使用 objectName 或动态属性选择器）
...
- [x] 3.3.2 保留的控件重命名为语义化名称（`hexSendBtn`、`hexDisplayBtn` 等）
- [x] 3.3.3 更新 `leftbar.h` 中的 getter 方法名（如 `recvHexBtn()` → `hexDisplayBtn()`）
- [x] 3.3.4 更新 `bytetracebase.cpp` 中所有引用

### 3.4 SettingsUI / SystemPage 代码重复清理 (#13, #14)
- [x] 3.4.1 移除 `settingsui.ui` 中 stackedWidget 的 5 个空白占位页
- [x] 3.4.2 移除 `settingsui.ui` 中未连接的 btnReturn 按钮
- [x] 3.4.3 重构 `SystemPage`：统一使用 .ui 文件或纯代码构建，移除双重构建
...
### 3.7 命名一致性 (#17)
- [x] 3.7.1 重命名 LeftBar 中所有带自动生成后缀的控件（`_7`、`_13` 等）
- [x] 3.7.2 检查全项目成员变量命名：统一 `m_` 前缀 + `m_p` 指针前缀
- [x] 3.7.3 修复 `trayIcon` 等缺少前缀的变量（`updatemanager.h`）
